...
(function () {
  ...
  var CONSTANTS = {
    ...
    URLS: {
      ...
      //STAGE: "https://app.digio.in",
      //SANDBOX: "https://app.digio.in",
  ...
  digioHandle.prototype = {
    ...
    URL: {},
    // logo: "https://info.smcindiaonline.org:5001/static/media/smclogo.11e5cb24.png", // SMC Logo
    logo: "https://info.stoxkart.com:5002/static/media/stox.b15d68cb.png", // Stoxkart Logo
    redirectUrl: null,
    errorUrl: null,
    isIframe: false,
    documentId: null,
    ...
    init: function (myValues) {
      console.log("VLAUES IN INIT", myValues);
      this.reset();
      ...
      if (this.isIframe) {
        ...
        iframeContainer.style.cssText =
          "position: absolute; min-width: 450px; overflow: auto; z-index : 999999; width : 100%; height : 100%; " +
        ...
        iframeWrapper.style.cssText =
          "position: absolute; min-width: 450px; overflow: auto; z-index : 999999; width : " +
          newWidth +
          "px; height : " +
          newHeight +
        ...
        try {
          if (that.isIframe && that.iFrameObj.closed) {
            ...
            if (that.resultCaptured) {
              ...
              const GET_URLS = {
                segment:
                  "https://api.stoxkart.com:8082/Segment/getSegmentEsign",
                mobile:
                  "https://api.stoxkart.com:8082/Mobile/getEsignDocuments",
                bank: "https://api.stoxkart.com:8082/Bank/getBankEsign",
                nominee: `${process.env.REACT_APP_BASE_URL}/Nominee/GetNomineeEsign`,
              };

              GET_URLS["email"] = GET_URLS["mobile"];
              GET_URLS["mobileAndEmail"] = GET_URLS["mobile"];
              if (!GET_URLS.hasOwnProperty(myValues.type)) {
                console.error(
                  `For Request type '${myValues.type}', URL is not configured. `
                );
              }
              let data = {
                userCode: myValues.userCode,
                userDevice: "Web",
                userEmail: myValues.userEmail,
                UserEmail: myValues.userEmail || myValues.userOldEmail,
                userOldEmail: myValues.userOldEmail,
                userIP: sessionStorage.getItem("ip"),
                userMobile: myValues.userMobile,
                userOldMobile: myValues.userOldMobile,
                userName: myValues.userName,
                userPan: myValues.PAN,
                userEmailId: myValues.userEmail,
                userMobileNumber: myValues.userMobile,
                userProduct: "ad",
                userUccCode: myValues.userCode,
                esignId: "" + that.documentId + "",
                esignFor:
                  myValues.type === "mobile"
                    ? "MOBILE"
                    : myValues.type === "email"
                    ? "EMAIL"
                    : myValues.type === "mobileandEmail"
                    ? "EMAILMOBILE"
                    : myValues.type === "nominee"
                    ? "NOMINEE"
                    : myValues.type === "bank"
                    ? "BANK"
                    : "EMAILMOBILE",
              };
              if (myValues.type === "nominee") {
                data.isOptedforNominee = myValues.isOptedforNominee;
              }
              var settings = {
                url: GET_URLS[myValues.type],
                method: "POST",
                credentials: "include",
                timeout: 0,
                headers: {
              ...
              $.ajax(settings).done(function (response) {
                ...
                if (myValues.type !== "nominee") {
                  setTimeout(function () {
                    window.location.pathname = "/updateContact";
                  }, 1000);
                }
                //                                     var win = window.open(response.data.docPath, '_blank');
                // if (win) {
              ...
              // $.ajax({
              //     type: "POST",
              //     url: "/AppESign/GetEasignPDF",
              //     data: "{docId:'" + that.documentId + "',DocType:'" + flag + "'}",
              ...
              //         }
              //         else {
              //             if (flag == "Account".trim()) {
              //                 $('#imgAccount').attr('src', '/img/tick.png');
              //                 $('#imgAccount').attr('width', '30');
              //                 $('#lblAccount').html('Esign');
              //                 $('#lblAccount').css('font-weight', 'Bold');
              //             }
              //             //else if (flag == "DP".trim()) {
              //             //    $('#imgDP').attr('src', '/img/tick.png');
              //             //    $('#imgDP').attr('width', '30');
              //             //    $('#lblDP').html('Esign');
              //             //    $('#lblDP').css('font-weight', 'Bold');
              //             //}
              //             else if (flag == "KRA".trim()) {
              //                 $('#imgKRA').attr('src', '/img/tick.png');
              //                 $('#imgKRA').attr('width', '30');
              //                 $('#lblKRA').html('Esign');
              //                 $('#lblKRA').css('font-weight', 'Bold');
              ...
              //         //console.log(response);
              //         alert("faliure on file upload" + JSON.stringify(response));
              //     }
              // });
              // console.log(that.result);

              if (that.result.message.toLowerCase() == "signed successfully") {
                // return {docid:that.result.digio_doc_id,message:that.result.message};
                // window.localStorage.setItem('docid',that.result.digio_doc_id)
              }
              // else{
              //     return {docid:'',message:that.result.message};
              // }
              that.callback(that.result);
            } else {
              resObj = {
    ...
    submit: function (ids, identifier, token_id, myValues) {
      try {
        ...
        if (Array.isArray(ids)) {
          return this.esign(ids, identifier, token_id, myValues);
        } else if (
          typeof ids === "string" &&
          ids.slice(0, 3) === "ENA" &&
          ids.slice(ids.length - 2, ids.length) === "AP"
        ) {
          return this.enachApiSign(ids, identifier, token_id, myValues);
        } else {
          return this.esign(ids, identifier, token_id, myValues);
        }
      } catch (err) {
    ...
    enachApiSign: function (id, identifier, token_id, myValues) {
      this.myValues = myValues;
      console.log("ENACHAPISIGN", myValues, this.myValues);
    ...
    esign: function (ids, identifier, token_id, myValues) {
      ...
      if (
        ...
        return this.enachApiSign(ids, identifier, myValues);
      }
